...
| M9 | Notifications + push | **done 2026-08-24** — §9.2's centre, badge, per-category preferences and row-to-target routing (1.10.0), then push: device registration, tap-to-route from a closed app and a foreground badge refresh (1.11.0). Reopened by the 1.4.1 audit as MT-005; the 1.11.0 audit confirmed the whole push path on a device and found **MT-020**, which is why 1.11.1 exists — both mark-read routes were `POST` against `@Put` and had never worked |
| M10 | Admin module (now including §10.5 wallet/payment administration) | **done 2026-08-25** — the dashboard 2026-08-21, all three moderation queues 2026-08-22, user search with warn/restrict/block/unblock 2026-08-23, the audit log the same day, dictionary management 2026-08-24, and §10.5's wallets, immutable ledger and BR-24 adjustment 2026-08-25. Every current-state counter on the dashboard leads to the screen it counts. **Three things are stated on screen rather than built, because each needs a route that does not exist**: §10.5's Payment Order search, editing the three money settings, and §10.3's label editing. All three are in docs/BACKEND_ASKS.md, and none of them blocks anything — top-up is not live either |
...
- [done 2026-08-20] The employer's half of §8.3 — schedule, move, call off, on
  §6.5's applicant row. One form for scheduling and rescheduling, because the
  server takes one shape for both; the picked time is the **platform's** wall
  clock and the offset is read from a server timestamp, never from a `+05:00` in
  Dart. What remains missing is only the **aggregate** list, since
...
- User search and warn/restrict/block/unblock with reason (UAT-14). *(done
  2026-08-23. The audit log is a separate endpoint and a separate screen —
