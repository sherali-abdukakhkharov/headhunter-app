...
| ID | Sev | State |
|---|---|---|
| MT-003 | Critical | **Code done `headhunter-backend@ad93231`; one deployment action left.** `MODERATION_ENABLED` and `EMPLOYER_VERIFICATION_ENABLED` are now `Joi.valid(true)` when `NODE_ENV=production`, so the container **refuses to boot** with either off — a warning was what three consecutive audits found being ignored. A smoke test submits a vacancy, asserts a candidate cannot find it, approves it and asserts they can. The five production refusals had no tests at all and now do. **The action: set both to `true` in the deployed host's `.env`, then `pnpm api:up`** — until then that instance will not start, which is the check working |
