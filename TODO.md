...
| P1 | Profile / Account | **Done — the entry in 1.31.0, the sections in 1.32.0.** The settings entry is a header action on both profile screens, present in the loading and error states too. The candidate profile is now a hub: one row per schema section, each saying what is missing in it and opening its own page, and **each page saves only its own fields** — the endpoint was already partial by field code, so no backend change. The rows are flat and in schema order **on purpose**: which sections exist depends on the work category and §10.3 lets an administrator add one at runtime, so a client-side grouping table would be wrong the first time that happened. The employer profile is left alone — it is one and a half screens |
...
| P2 | Admin navigation | **Done 1.31.0, and the rename declined.** The soft hyphen moved: it split `Foydalanuvchilar` mid-syllable at `la-nuv`, and it breaks after `nuv` now. **`Hisoblar` was put to the owner on 2026-08-28 and refused** — the tab lists people, and a shorter word that means something else is not a shorter word for the same thing. The label stays; a later audit should not re-propose it |
